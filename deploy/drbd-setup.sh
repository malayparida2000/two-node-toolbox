...
#     host podman uses --authfile /var/lib/kubelet/config.json. ${DRBD_PORT}/tcp open between nodes.
...
DRBD_IMAGE="${DRBD_IMAGE:-quay.io/rhceph-dev/odf4-odf-drbd-rhel9:v4.22.0}" # ODF DRBD image (drbdadm + sources)
# TODO: bump when tarball inside the image changes.
DRBD_VERSION="${DRBD_VERSION:-9.2.18}"                                   # Must match DRBD source version in DRBD_IMAGE
...
# Host podman does not read the cluster pull-secret by default; pass kubelet's auth file
# so registry.redhat.io (and other authenticated registries) can be pulled.
drbdctl() {
    local node="$1"
    shift
